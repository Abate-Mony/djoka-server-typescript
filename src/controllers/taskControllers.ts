...
export const getStaticTask: MiddlewareFn = async (req, res) => {
  const id = req.params.id;
  const task = await Task.findOne({ task_id: id });
  if (!task) throw new NotFoundError("couldnot find task with id");
  res.status(StatusCodes.OK).json({ task });
};
export const getTasks: MiddlewareFn = async (req, res) => {
  const { userId, role } = req.user;
...
export const deleteTask: MiddlewareFn = async (req, res) => {
  const id = req.params.id;
  ...
  if (!tasks) throw new BadRequestError(`fail to delete tasks with ${id}`);
...
export const updateTask: MiddlewareFn = async (req, res) => {
  ...
  if (!tasks) throw new BadRequestError("fal to update tasks with id");
